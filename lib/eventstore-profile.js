...
function parseProfileFromMessage(msg) {
  if (!msg) return null;
  // 兼容多种回包形态：event.data / event.content / 直接 JSON 字符串 / 包裹在 message.event 中
  const candidates = [];
  if (typeof msg === 'string') candidates.push(msg);
  if (msg && typeof msg === 'object') {
    candidates.push(msg.data, msg.content, msg.profile);
    if (msg.event && typeof msg.event === 'object') {
      candidates.push(msg.event.data, msg.event.content, msg.event.profile);
    }
  }
  let data = null;
  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      mod.get_user_profile(pubkey, (msg) => {
        if (resolved) return;
        try {
          const profile = parseProfileFromMessage(msg);
          ...
          // 某些服务器先回状态包（无 data），继续等真正 profile；错误码则立即返回
...
async function saveProfile(_esserver, pubkey, privkeyEsec, profile) {
  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      mod.save_user_profile(profile || {}, pubkey, privkeyBytes, () => {
        clearTimeout(t);
        resolve({ ok: true });
      });
    });
