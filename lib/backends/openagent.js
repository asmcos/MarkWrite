...
/** 方舟通用推理用 api/v3；编程专线为 api/coding/v3（见文档）。deepseek-v3.2 为无效简称。 */
const VOLCENGINE_ARK_BASE = 'https://ark.cn-beijing.volces.com/api/v3';
const VOLCENGINE_MODEL_FALLBACK = 'deepseek-v3-2-251201';

function normalizeVolcengineModelId(modelId) {
  if (modelId == null || typeof modelId !== 'string') return modelId;
  const t = modelId.trim();
  if (t === 'deepseek-v3.2' || t === 'deepseek3.2' || t === 'deepseek-v3-2') {
    return VOLCENGINE_MODEL_FALLBACK;
  }
  return t;
}
...
function ensureConfig(configPath) {
  ...
  try {
    ...
    fs.writeFileSync(configFile, JSON.stringify({
      volcengine: {
        name: 'volcengine',
        options: { baseURL: VOLCENGINE_ARK_BASE, apiKey: '在此填写你的 API Key' },
        models: { 'deepseek-v3-2-251201': { name: 'deepseek-v3-2-251201' } },
      },
    }, null, 2), 'utf8');
    return true;
  } catch (e) {
...
async function getAgentState() {
  ...
  let modelId = options.modelId || (prefix ? process.env[`${prefix}_MODEL`] : null) || process.env.OPENAGENT_MODEL || cfg.modelId;
  if (!modelId) {
    throw new Error('未配置 modelId，请在配置中设置 openagent.modelId 或环境变量 OPENAGENT_MODEL');
  }
  if (resolvedKey === 'volcengine') {
    modelId = normalizeVolcengineModelId(modelId);
  }
